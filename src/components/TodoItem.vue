    ...
    <div class="row my-3 justify-content-between align-items-center">
      ...
      <div v-else class="mr-2 row justify-content-between align-items-center col">
        ...
        <div>
          ...
          <label class>Terminado</label>
        </div>
      </div>
      <div class="row align-items-center">
        <button @click="updateTodoI(todo)" class="btn btn-primary mx-2">{{editing?'Update':'Editar'}}</button>
        <button @click="deleteTodo(todo.id)" class="btn btn-danger">Eliminar</button>
      </div>
    </div>
  </div>
</template>
